...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle Proxmox VE Helper Scripts: ToxicWeb Edition v0.1.0 --inputbox "Укажите имя хоста(hostname)" 8 58 turnkey-owncloud-vm --title "ИМЯ ХОСТА(HOSTNAME)" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="turnkey-owncloud-vm"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
