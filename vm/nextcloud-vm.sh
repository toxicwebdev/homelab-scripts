...
if whiptail --backtitle Proxmox VE Helper Scripts: ToxicWeb Edition v0.1.0 --title "$NAME" --yesno "This will create a New $NAME. Proceed?" 10 58; then
  :
else
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle Proxmox VE Helper Scripts: ToxicWeb Edition v0.1.0 --inputbox "Укажите имя хоста(hostname)" 8 58 turnkey-nextcloud-vm --title "ИМЯ ХОСТА(HOSTNAME)" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="$HN"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if (whiptail --backtitle Proxmox VE Helper Scripts: ToxicWeb Edition v0.1.0 --title "РАСШИРЕННЫЕ НАСТРОЙКИ ЗАВЕРШЕНЫ" --yesno "Ready to create a $NAME?" --no-button Do-Over 10 58); then
...
if [ "$START_VM" == "yes" ]; then
  msg_info "Запускаю $NAME"
  qm start $VMID
  msg_ok "Запустил $NAME"
fi
