...
if whiptail --backtitle Proxmox VE Helper Scripts: ToxicWeb Edition v0.1.0 --title "Ubuntu 22.04 VM" --yesno "This will create a New Ubuntu 22.04 VM. Proceed?" 10 58; then
  :
else
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle Proxmox VE Helper Scripts: ToxicWeb Edition v0.1.0 --inputbox "Укажите имя хоста(hostname)" 8 58 ubuntu --title "ИМЯ ХОСТА(HOSTNAME)" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="ubuntu"
      echo -e "${DGN}Using Hostname: ${BGN}$HN${CL}"
    else
  ...
  if (whiptail --backtitle Proxmox VE Helper Scripts: ToxicWeb Edition v0.1.0 --title "РАСШИРЕННЫЕ НАСТРОЙКИ ЗАВЕРШЕНЫ" --yesno "Ready to create an Ubuntu 22.04 VM?" --no-button Do-Over 10 58); then
...
if [ "$START_VM" == "yes" ]; then
  msg_info "Запускаю Ubuntu 22.04 VM"
  qm start $VMID
  msg_ok "Запустил Ubuntu 22.04 VM"
fi
